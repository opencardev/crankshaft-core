...
bool PreferencesService::initialize() {
  if (m_dbPath != QStringLiteral(":memory:")) {
    const QFileInfo dbFileInfo(m_dbPath);
    const QString dbDirectoryPath = dbFileInfo.absolutePath();

    if (!dbDirectoryPath.isEmpty()) {
      QDir dbDirectory(dbDirectoryPath);
      if (!dbDirectory.exists() && !dbDirectory.mkpath(QStringLiteral("."))) {
        Logger::instance().error(
            QString("[PreferencesService] Failed to create database directory: %1")
                .arg(dbDirectoryPath));
        return false;
      }
    }
