...
using DBusInterfacePropertyMap = QMap<QString, QVariantMap>;
using DBusManagedObjectMap = QMap<QDBusObjectPath, DBusInterfacePropertyMap>;
...
class BluetoothHAL::BluetoothHALPrivate {
 public:
  ...
  BluetoothHALPrivate() : dbusConnection(QDBusConnection::systemBus()) {
    qDBusRegisterMetaType<DBusInterfacePropertyMap>();
...
bool BluetoothHAL::BluetoothHALPrivate::findAdapter() {
  ...
  QDBusReply<DBusManagedObjectMap> reply =
  ...
  DBusManagedObjectMap objects = reply.value();
...
BluetoothDevice BluetoothHAL::BluetoothHALPrivate::parseDevice(const QString& devicePath,
  ...
  device.type = BluetoothDevice::DeviceType::Unknown;
  device.rssi = -100;
  ...
  if (properties.contains("UUIDs")) {
    const QVariant uuidValue = properties["UUIDs"];
    if (uuidValue.canConvert<QStringList>()) {
      device.type = guessDeviceType(uuidValue.toStringList());
    }
  }
...
BluetoothHAL::State BluetoothHAL::getState() const {
  ...
  const QVariant poweredVar = adapter.property("Powered");
  if (!poweredVar.isValid()) {
    return State::Off;
  }

  return poweredVar.toBool() ? State::On : State::Off;
}

QString BluetoothHAL::getLocalAddress() const {
  if (d->adapterPath.isEmpty()) {
    return {};
  }

  QDBusInterface adapter(BLUEZ_SERVICE, d->adapterPath, BLUEZ_ADAPTER_INTERFACE, d->dbusConnection);
  return adapter.isValid() ? adapter.property("Address").toString() : QString();
}

QString BluetoothHAL::getLocalName() const {
  if (d->adapterPath.isEmpty()) {
    return {};
  }

  QDBusInterface adapter(BLUEZ_SERVICE, d->adapterPath, BLUEZ_ADAPTER_INTERFACE, d->dbusConnection);
  return adapter.isValid() ? adapter.property("Alias").toString() : QString();
}

bool BluetoothHAL::setLocalName(const QString& name) {
  if (d->adapterPath.isEmpty()) {
    return false;
  }
  ...
  adapter.setProperty("Alias", name);
  return true;
}

bool BluetoothHAL::isDiscovering() const {
  return d->discovering;
}

bool BluetoothHAL::isEnabled() const {
  if (d->adapterPath.isEmpty()) {
    auto* mutablePrivate = const_cast<BluetoothHALPrivate*>(d);
    if (!mutablePrivate->findAdapter()) {
      return false;
    }
  }
...
bool BluetoothHAL::setEnabled(bool enabled) {
  if (d->adapterPath.isEmpty() && !d->findAdapter()) {
    qWarning() << "No Bluetooth adapter available";
    emit errorOccurred("No Bluetooth adapter available");
    return false;
  }
...
QList<BluetoothDevice> BluetoothHAL::getPairedDevices() const {
  QList<BluetoothDevice> paired;
...
bool BluetoothHAL::pairDevice(const QString& deviceAddress) {
  ...
  // Enable pairable mode on the adapter before pairing
  if (!d->adapterPath.isEmpty()) {
    QDBusInterface adapter(BLUEZ_SERVICE, d->adapterPath, BLUEZ_ADAPTER_INTERFACE, d->dbusConnection);
    if (adapter.isValid()) {
      // Set adapter to pairable mode with a 5-minute timeout
      adapter.setProperty("Pairable", true);
      adapter.setProperty("PairableTimeout", 300);
      qDebug() << "Adapter set to pairable mode for device:" << deviceAddress;
    }
  }
...
bool BluetoothHAL::isConnected(const QString& address) const {
  for (const auto& device : d->devices) {
