...
static const char* NM_DBUS_INTERFACE_ACTIVE_CONNECTION =
...
class WiFiHAL::WiFiHALPrivate {
 public:
  ...
  QString hotspotActivePath;
  QString hotspotSSID;
  WiFiSecurity hotspotSecurity{WiFiSecurity::WPA2_PSK};
...
WiFiHAL::ConnectionState WiFiHAL::getConnectionState() const {
  if (d->wirelessDevicePath.isEmpty()) {
  ...
  const QVariant stateVar = device.property("State");
  if (!stateVar.isValid()) {
    return ConnectionState::Unknown;
  }

  switch (stateVar.toInt()) {
...
bool WiFiHAL::forgetNetwork(const QString& ssid) {
  QDBusInterface settings(NM_DBUS_SERVICE, "/org/freedesktop/NetworkManager/Settings",
                          NM_DBUS_INTERFACE_SETTINGS, d->dbusConnection);
  if (!settings.isValid()) {
    return false;
  ...
  if (!reply.isValid()) {
    qWarning() << "Failed to list saved connections:" << reply.error().message();
    return false;
  }

  bool removed = false;
  for (const auto& path : reply.value()) {
    QDBusInterface connection(NM_DBUS_SERVICE, path.path(), NM_DBUS_INTERFACE_CONNECTION,
                              d->dbusConnection);
    if (!connection.isValid()) {
      continue;
    }

    QDBusReply<QVariantMap> settingsReply = connection.call("GetSettings");
    if (!settingsReply.isValid()) {
      continue;
    }

    const QVariantMap connectionMap = settingsReply.value().value("connection").toMap();
    if (connectionMap.value("id").toString() == ssid) {
...
QStringList WiFiHAL::getSavedNetworks() const {
  QDBusInterface settings(NM_DBUS_SERVICE, "/org/freedesktop/NetworkManager/Settings",
                          NM_DBUS_INTERFACE_SETTINGS, d->dbusConnection);
  if (!settings.isValid()) {
    return {};
  ...
  for (const auto& path : reply.value()) {
    ...
    const QString id = settingsReply.value().value("connection").toMap().value("id").toString();
    if (!id.isEmpty()) {
      saved.append(id);
    }
  }
...
bool WiFiHAL::startHotspot(const HotspotConfig& config) {
  if (d->wirelessDevicePath.isEmpty()) {
    return false;
  }

  if (config.ssid.trimmed().isEmpty()) {
    qWarning() << "Cannot start hotspot: SSID is empty";
    return false;
  }
  ...
  if (!nm.isValid() || !settings.isValid()) {
    qWarning() << "NetworkManager interfaces not available for hotspot setup";
    return false;
  }
  ...
  QVariantMap connection;
  connection["id"] = QStringLiteral("Crankshaft-WirelessAA-Hotspot");
  connection["type"] = "802-11-wireless";
  connection["uuid"] = QUuid::createUuid().toString().remove('{').remove('}');
  connection["autoconnect"] = false;

  QVariantMap wireless;
  wireless["ssid"] = config.ssid.toUtf8();
  wireless["mode"] = "ap";
  if (config.channel > 0) {
    wireless["band"] = "bg";
    wireless["channel"] = config.channel;
  }
  ...
  if (config.security != WiFiSecurity::Open) {
    if (config.password.size() < 8) {
      qWarning() << "Cannot start hotspot: WPA password must be at least 8 characters";
      return false;
    }

    QVariantMap wirelessSecurity;
    wirelessSecurity["key-mgmt"] = "wpa-psk";
    wirelessSecurity["psk"] = config.password;
    connectionSettings["802-11-wireless-security"] = wirelessSecurity;
  }
  ...
  addReply.waitForFinished();
  if (addReply.isError()) {
    qWarning() << "Failed to add hotspot connection:" << addReply.error().message();
    return false;
  }

  const QDBusObjectPath connectionPath = addReply.value();
  QDBusPendingReply<QDBusObjectPath> activateReply =
      nm.asyncCall("ActivateConnection", QVariant::fromValue(connectionPath),
                   QVariant::fromValue(QDBusObjectPath(d->wirelessDevicePath)),
                   QVariant::fromValue(QDBusObjectPath("/")));
  activateReply.waitForFinished();
  if (activateReply.isError()) {
    qWarning() << "Failed to activate hotspot connection:" << activateReply.error().message();
    QDBusInterface hotspotConn(NM_DBUS_SERVICE, connectionPath.path(), NM_DBUS_INTERFACE_CONNECTION,
                               d->dbusConnection);
    if (hotspotConn.isValid()) {
      hotspotConn.call("Delete");
    }
    return false;
  }

  d->hotspotConnectionPath = connectionPath.path();
  d->hotspotActivePath = activateReply.value().path();
  d->hotspotSSID = config.ssid;
  d->hotspotSecurity = config.security;
  d->connectedSSID = config.ssid;

  qDebug() << "Hotspot started:" << d->hotspotSSID << "active path:" << d->hotspotActivePath;
  return true;
}

void WiFiHAL::stopHotspot() {
  if (d->hotspotActivePath.isEmpty() && d->hotspotConnectionPath.isEmpty()) {
    return;
  }

  QDBusInterface nm(NM_DBUS_SERVICE, NM_DBUS_PATH, NM_DBUS_INTERFACE, d->dbusConnection);
  if (nm.isValid() && !d->hotspotActivePath.isEmpty()) {
    nm.call("DeactivateConnection", QVariant::fromValue(QDBusObjectPath(d->hotspotActivePath)));
  }

  if (!d->hotspotConnectionPath.isEmpty()) {
    QDBusInterface connection(NM_DBUS_SERVICE, d->hotspotConnectionPath,
                              NM_DBUS_INTERFACE_CONNECTION, d->dbusConnection);
    if (connection.isValid()) {
      connection.call("Delete");
    }
  }

  d->hotspotActivePath.clear();
  d->hotspotConnectionPath.clear();
  d->hotspotSSID.clear();
}

WiFiHAL::HotspotStatus WiFiHAL::getHotspotStatus() const {
  HotspotStatus status;
  status.active = !d->hotspotActivePath.isEmpty();
  status.ssid = d->hotspotSSID;
  status.security = d->hotspotSecurity;
  ...
  const QVariant hwAddress = device.property("HwAddress");
  if (hwAddress.isValid()) {
    status.bssid = hwAddress.toString();
  }

  if (!d->hotspotActivePath.isEmpty()) {
    QDBusInterface activeConnection(NM_DBUS_SERVICE, d->hotspotActivePath,
                                    NM_DBUS_INTERFACE_ACTIVE_CONNECTION, d->dbusConnection);
    if (activeConnection.isValid()) {
      const QVariant stateVar = activeConnection.property("State");
      if (stateVar.isValid()) {
        status.active = (stateVar.toInt() == 2);  // NM_ACTIVE_CONNECTION_STATE_ACTIVATED
      }
    }
  }
